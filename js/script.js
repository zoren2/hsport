...
(function () {
	...
	var state = {};
	var category = 'all'; // get weather data when user clicks Forecast button, then add temp & conditions to view
	...
	document.querySelector('.forecast-button').addEventListener('click', function (e) {
		...
		var location = document.querySelector('#location').value; //		$('#location').val('');
	...
	function updateUISuccess(response) {
		...
		var into = document.querySelector('.conditions'); //		ReactDOM.render(<Forecast {...state} />, $into);
        ...
        */
		...
		var conditionsPara = document.createElement('p'); //		conditionsPara.textContent = state.degCInt + '\u00B0 C / ' + state.degFInt + '\u00B0 F';

		conditionsPara.textContent = "".concat(state.degCInt, "\xB0 C / ").concat(state.degFInt, "\xB0 F");
	...
	function updateActivityList(event) {
		...
		var into = document.querySelector('.activities'); //		ReactDOM.render(<Activities {...state} />, $into);
